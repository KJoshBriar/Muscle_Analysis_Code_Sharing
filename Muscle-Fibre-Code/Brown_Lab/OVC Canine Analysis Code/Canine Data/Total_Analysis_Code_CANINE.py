...
import warnings
import openpyxl
import KTR_STIFF_CANINE
import Unloaded_Short_CANINE
...
def ReadFile(File: str = None, model: str = None, Filename = None, test: str = None):
    ...
    if test == 'ktr':
        ...
        Velocity: str = Filename[2]
        Fibre: str = Filename[3]
        data_start = Data.index[Data['Time'] == '50000.00'][0]
        Data = pd.DataFrame(Data[data_start:], dtype = float).reset_index(drop = True)
        Baseline_force = Data['Force'][1:500].mean()
        Data['Force'] = Data['Force'] - Baseline_force
        Data['Time'] = Data['Time'].div(1000)
...
def main():
    ...
    for file in Directory:
        ...
        if test == 'unloaded':
            results['Time of force redevelopment'], results['Isometric Force']=Unloaded_Short_CANINE.VelocityAnalysis(Data = results['data'], Filename = os.path.basename(file), CSA=results['CSA'], Graph = True)
        if test == 'ktr':
            results['Stiffness (pCa 4.5)'], results['Relaxing Stiffness'], results['Modulus (4.5)'], results['Modulus (Relaxing)'], results['Normalized Modulus'], results['ktr (pCa 4.5)'], results ['Goodness of fit'], results ['Active Specific Force'], results['KTR Delay'] =KTR_STIFF_CANINE.ktrAnalysis(Data = results['data'], Filename = os.path.basename(file), CSA=results['CSA'], Graph = False
                                                                                                                                                                                                                                                                                                                )
...
excel_file_path = '/Users/keaton.briar/Desktop/Single Fibre Data/Canine Study/Outputd4.xlsx'
