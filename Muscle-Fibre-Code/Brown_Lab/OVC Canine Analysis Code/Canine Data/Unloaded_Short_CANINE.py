    ...
    def Force_Positive(Data: pd.DataFrame, UnloadedShortening_Time: int = UnloadedShortening_Time):
        ...
        def handle_dem_mouse_clickies(): #Code to define interactable graph
         ...
         #print (click_x-250000)
            ...
            ax.plot(Data['Time'][click_x-500000:UnloadedShortening_Time+10000], Data['Force'][click_x-500000:UnloadedShortening_Time+10000], color='red') #plot graph data from the pre-selected model data of the KTR time
            ax.axvline(x=Data['Time'][click_x-500000], ymin = 0, ymax = 1, color = 'Green', linestyle = '-', linewidth = 2) #show cursor line
            fig.canvas.draw()
            plt.show()
        ...
        for index in positive_window:
            ...
            #add a print
                return Positive_Force 
 # Return the index when force becomes positive
    ...
    return (Positive_force-580000)/10, Peak_force/CSA
