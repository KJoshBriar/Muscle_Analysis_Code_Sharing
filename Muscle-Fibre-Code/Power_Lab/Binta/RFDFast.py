...
def ktrAnalysis(Data: pd.DataFrame = None, Filename: str = None, CSA: float = None, FibreLength: float = None, Graph: bool = False) -> tuple[float, float, float, pd.Series, pd.Series, float]:
    ...
    def Work_calculate(Data: pd.DataFrame, work_time_start: int = work_time_start, FibreLength: float = None):
        work = 0
        work_Window = range(int(work_time_start), int((work_time_start)+((FibreLength*0.15)*10000))) #this window specifies the range
        Work_Time = (int(work_time_start)+(((FibreLength*0.15)*10000))) - (int(work_time_start)) #time for how long the work goes on for
        ...
        delta_length = (Data['Length'][int(work_time_start)+((FibreLength*0.15)*10000)]) - (Data['Length'][work_time_start])  # Change in length
